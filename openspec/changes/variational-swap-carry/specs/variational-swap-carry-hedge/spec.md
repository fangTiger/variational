...
### Requirement: 积分权重择优（2026-09-08 更新）
系统 SHALL 使用本节替代此前按时段指定唯一目标或仅按 carry 排序的选择规则；既有风控优先级保持不变。
系统 SHALL 支持 XAUS_XAU、XAU_XAUT、XAUS_XAUT 和 TRIPLE（1:1:2）。
POINTS_WEIGHTS 默认为 XAUS=2、XAU=1、XAUT=1；来源为用户确认的官方公告，API 无法验证。
积分 OI SHALL 按多空各腿名义绝对值乘权重求和。候选与切换增益按同一配置的基础名义比较；面板另展示实际持仓积分 OI。
系统 SHALL 先过滤交易窗口、费率读取失败、净 carry < 2%、所需保证金 > 可用保证金 × 60% 的结构，
再按积分 OI 降序、carry 降序排序。XAUS 所需保证金 SHALL 包含补到目标强平距离所需的桶，
取报价 IM 与目标桶的较大值。不可报价时金额标记未知，不将缺失值作为零。
既有下单名义硬上限仍由执行层拦截，不因积分择优放宽。

#### Scenario: 实测费率与预算过滤
- **WHEN** 每基础腿名义 4000，XAU 年化 2.59%、XAUT 年化 10.95%、XAUS 多头成本 5.70%
- **THEN** 保证金预算充足时选积分 OI 20000 的 TRIPLE
- **AND** 预算仅 618 时排除 TRIPLE，选积分 OI 12000 的 XAUS_XAUT
- **AND** XAUS_XAU 因负 carry 被过滤

#### Scenario: 切换滞回与安全例外
- **WHEN** 已持仓且目标不同
- **THEN** 积分 OI 增益至少 20% 或 carry 优势至少年化 2.5%，且距上次切换至少 4 小时才允许普通切换
- **AND** 长休市 lead time 内强制切出 XAUS，忽略上述择优与滞回限制；无法开安全结构时保留原有强制平仓路径

#### Scenario: 可解释记录
- **WHEN** 完成候选评估
- **THEN** 心跳与审计记录各候选 carry、积分 OI、保证金、过滤原因、入选标记、名次及积分或 carry 胜出的依据
- **AND** 面板显示当前结构与最优结构的同规模积分 OI，并显示实际持仓积分 OI

### Requirement: 重开冷却仅用于风控退出
守护进程 SHALL 区分 `risk`、`switch`、`scheduled` 平仓来源。只有真实的风控平仓
更新状态文件中的 `last_closed_at`；结构切换及长休市计划平仓 SHALL 不更新该字段。
既有普通开仓的失败回滚冷却保持不变。

#### Scenario: 切换平仓与开仓不可被防抖拆断
- **WHEN** 切换已通过准入并开始执行
- **THEN** 平仓前持久化切换来源与目标，全平后同轮继续原目标开仓
- **AND** 后续开仓不再受重开冷却、收益滞回、最短持有期或新开仓次数上限阻断
- **AND** 保留 kill switch、事故标记、市场可交易性及真实保证金和执行校验

#### Scenario: 真实开仓失败后续跑
- **WHEN** 旧结构已全平，但目标因保证金不足、偏斜拒单或报价失败未开成
- **THEN** 明确记录切换开仓失败并告警，不把跳过动作的零返回码当成开仓成功
- **AND** 空仓且没有风险事故时，下一轮继续原目标，不重新择优或等待冷却
- **AND** 目标持仓自检成功后清除未完成切换并记录切换完成时间

#### Scenario: 重置立即影响下一轮
- **WHEN** 执行 `--reset-reopen-cooldown`
- **THEN** 只修改本地状态文件，清空冷却并保留重置记录及其它状态
- **AND** 下一轮执行完整判定，不复制旧心跳中的冷却结论
- **AND** 冷却时间只从状态文件读取，运行中旧快照不得覆盖已读到的重置
