...
#
# 这条阈值只兜底「连接活着但应用层不发数据」这一种罕见情况：断线与进程
# 重启已由 mark_discontinuity() 显式标记，而 websockets 默认
# ping_interval=20/ping_timeout=20，连接真死了 40 秒内就会抛异常触发重连、
# 同样被显式标记。所以它不需要（也不该）去猜市场静默。
#
# 取值依据（实测）：60 秒时 3.7 小时内误报 12 次，日志证明期间连接零中断，
# 全部是真实市场静默。误报的代价不是浪费数据那么简单——宽格距上一次转折
# 需要价格单向走完一整格再反转，耗时几分钟、天然跨越静默期，在那里切一刀
# 会把真实转折劈成两半双双不计。实测这 12 次误报把操作点 α 从 1.406 抬到
# 2.811，跨过中性区两侧，导出完全相反的交易决策。
# 观测到的静默期最长 170 秒，取 300 秒留近 2 倍余量。
DEFAULT_MAX_GAP_MS = 300_000
