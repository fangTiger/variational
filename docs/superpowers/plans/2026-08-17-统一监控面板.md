...
**更正**：初稿写的是「不增加 API 调用」，那是错的——下面给的代码会额外调一次 `hedge.get_balance()` 和一次 `primary.get_collateral()`，**每轮多 2 次调用**。评估后仍按此实现：每 30 秒 2 次、增加约 0.5–1 秒延迟；最坏情况两个接口都超时约 20 秒，周期拉到 50 秒仍低于 90 秒的心跳陈旧阈值，不会误报。想省掉重复的 `get_balance()` 就得改 `get_free_margin_ratio()` 的取数路径，那是保证金告警的依赖，属风控相关代码，为省一次 300ms 调用去动它不划算。
