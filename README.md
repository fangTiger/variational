...
> **另有一套同所 carry 对冲子系统**（在同一个 Variational 账户内对同一底层
> 持有反向双腿，赚资金费差而非成交量积分），文档见
> **[docs/swap-carry/README.md](docs/swap-carry/README.md)**。
...
| 对 | 主腿（积分来源） | 对冲腿 | 单腿名义额 | 周期 |
|---|---|---|---|---|
| **SNDK** | `io:SNDK`（Entropy dex） | `xyz:SNDK`（XYZ dex） | $2,000 | 12 小时 |
| **BTC** | `Lighter:BTC` | `Variational:BTC` | $3,500~4,000 | 8 小时 |

SNDK 这一对的两条腿**都在 Hyperliquid 上**（io 与 xyz 是两个 builder dex），
共用同一个钱包与保证金池，无需额外开户或划转。

### 为什么两条腿的杠杆不一样

| | 保证金模式 | 杠杆 | 强平距离 |
|---|---|---|---|
| `io:SNDK` | **强制逐仓**（`strictIsolated`） | 5x | 约 15.8% |
| `xyz:SNDK` | 全仓 | 10x | 约 13.0% |

`io:SNDK` 被交易所限定为逐仓，**爆仓时无法调用现货余额补保证金**，
安全完全取决于分配给它的保证金。若也用上限 10 倍，强平距离只有约 4.8%，
而实测 SNDK 在 12 小时窗口内触及 4.8% 单向逆向波动的概率高达 **20.9%**。
故 io 腿刻意降到 5 倍。`xyz` 是全仓，同样倍数下实际保护更强，
所以两腿的强平距离反而接近。

### 保证金与额度的对应关系

io 按 5 倍占名义额的 20%、xyz 按 10 倍占 10%，合计 **30%**，
另需约 25% 缓冲承受浮亏：
...
即「每投入 1 份抵押，约可支撑 2.6 份的单腿名义额」。
加大额度**不改变单位成交量成本**（已用大样本报价数据验证：
Variational 报价在 1~8 倍区间平坦，xyz 前三档深度充裕），
所以额度应由保证金安全决定，而不是由成本决定。

### 适配器组合

```bash
--primary-venue {lighter, variational, hyperliquid}
...
`tools/hedge_panel.py` 是零凭据的本地只读面板（默认 8787 端口），
除持仓、累计盈亏、成交量外，还提供**保证金与强平距离**区块：

```bash
python -m tools.hedge_panel --port 8787 --margin-address <公开账户地址>
```

每条腿显示杠杆、保证金模式、占用保证金、强平价与**强平距离**，并按风险配色：
...
`--margin-address` 只用于调用**只读公开行情接口**，不涉及任何签名凭据；
留空则不渲染该区块。该接口失败时页面其余部分照常渲染，
只在这一块显示失败原因——面板是监控工具，可用性优先。
